...
export class AkiflowClient {
  ...
  private readonly EVENTS_WRITE_URL = "https://api.akiflow.com/v3/events";
  ...
  async createEvent(event: {
    ...
    if (!calendar) {
      throw new Error(
        `Calendar ${event.calendar_id} not found. Use get-calendars to list valid calendars.`,
      );
    }
    if (calendar.read_only) {
      throw new Error(
        `Calendar "${calendar.title}" is read-only. Pick a writable calendar.`,
      );
    }

    const allDay = event.all_day ?? false;
    const toUtc = (s: string) =>
      `${new Date(s).toISOString().slice(0, 19)}.000Z`;
    ...
    const result = this.asList<Event>(
      await this.request("POST", this.EVENTS_WRITE_URL, [newEvent]),
    );
  ...
  async updateEvents(events: Partial<Event>[]): Promise<Event[]> {
    ...
    await this.syncStore.init();
    let cached = this.syncStore.getV5State<Event>("events").itemsById;
    if (events.some((e) => !cached[e.id as string])) {
      await this.getEvents();
      cached = this.syncStore.getV5State<Event>("events").itemsById;
    }
    ...
    const payload = events.map((e: any) => {
      const existing = cached[e.id as string];
      if (!existing) {
        throw new Error(
          `Event ${e.id} not found. Use get-events to load it before editing.`,
        );
      }
      const merged: any = { ...existing };
      if (e.title !== undefined) merged.title = e.title;
      if (e.description !== undefined) merged.description = e.description;
      if (e.location !== undefined) merged.location = e.location;
      const allDay = e.all_day ?? (merged as any).all_day ?? false;
    ...
    const result = this.asList<Event>(
      await this.request("POST", this.EVENTS_WRITE_URL, payload),
    );
    await this.mergeV5Items<Event>("events", result, (event) => !!event.deleted_at);
    return result;
  }
