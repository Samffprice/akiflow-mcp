...
import { AkiflowClient, } from "./akiflow-client.js";
import { z } from "zod";
// Validate environment
const REFRESH_TOKEN = process.env.AKIFLOW_REFRESH_TOKEN;
if (!REFRESH_TOKEN) {
    console.error("Error: AKIFLOW_REFRESH_TOKEN environment variable is required");
    process.exit(1);
}
// Initialize client
...
const UnscheduleTaskSchema = z.object({
    ...
    to_inbox: z
        .boolean()
        .optional()
        .describe("Move to inbox (status=1) instead of just removing time (default: true)"),
});
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "get-tasks",
            description: "Get tasks with optional filters. Returns active (non-deleted, non-trashed) tasks sorted by date.",
            inputSchema: {
                type: "object",
                properties: {
                    done: {
                    ...
                    status: {
                        type: "string",
                        enum: ["1", "2", "4", "7", "10"],
                        description: "1=Inbox, 2=Planned, 4=Snoozed, 7=Someday, 10=Scheduled",
                    },
                    date_from: {
                        type: "string",
                        description: "Only tasks on or after this date (YYYY-MM-DD). Compares against task date/datetime.",
                    },
                    date_to: {
                        type: "string",
                        description: "Only tasks on or before this date (YYYY-MM-DD). Compares against task date/datetime.",
                    },
                    listId: {
                        type: "string",
        ...
        {
            name: "get-events",
            description: "Get calendar events (v5 API). Returns detailed event data including attendees, location, recurrence.",
            inputSchema: {
                type: "object",
                properties: {
                    limit: {
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    datetime: {
                        type: "string",
                        description: "Specific time (ISO 8601, e.g. 2026-01-26T10:00:00.000Z)",
                    },
                    duration: {
                        type: "number",
        ...
        },
        {
            ...
            description: "Remove a task from the calendar and optionally move to inbox",
            inputSchema: {
                type: "object",
                properties: {
                    id: { type: "string", description: "Task UUID" },
        ...
        {
            name: "get-projects",
            description: "List all projects and folders. Note: Folders have type='folder', projects/labels have type=null",
            inputSchema: {
                type: "object",
                properties: {},
            },
        ...
        {
            name: "get-calendars",
            description: "Get all calendars. Returns calendar metadata including colors, settings, and sync status.",
            inputSchema: {
                type: "object",
                properties: {},
            },
        ...
        {
            name: "add-event",
            description: "Create a new calendar event. Syncs to the source calendar (Google, etc.)",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "edit-event",
            description: "Edit a calendar event. Changes sync back to the source calendar (Google, etc.)",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "get-time-slots",
            description: "Get time slots (internal calendar blocks that don't sync to external calendars)",
            inputSchema: {
                type: "object",
                properties: {
                    ...
                    date_from: {
                        type: "string",
                        description: "Only time slots starting on or after this date (YYYY-MM-DD)",
                    },
                    date_to: {
                        type: "string",
                        description: "Only time slots starting on or before this date (YYYY-MM-DD)",
                    },
                },
            },
        },
        {
            name: "add-time-slot",
            description: "Create a time slot (internal calendar block). Does not sync to external calendars.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Add a task to a time slot. Links the task to an existing time slot block on the calendar.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Remove a task from its time slot. Unlinks the task from its associated time slot block.",
            inputSchema: {
                type: "object",
                properties: {
                    task_id: {
        ...
        {
            name: "get-recordings",
            description: "List meeting recordings from Meeting Assistant. Each recording has a summary, action items, transcript, and brief.",
            inputSchema: {
                type: "object",
                properties: {
                    date_from: {
        ...
        {
            name: "get-recording",
            description: "Get full detail for a single meeting recording including summary, action items, raw transcript, and brief.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "List pre-meeting research briefs. Briefs provide background context and attendee info before meetings.",
            inputSchema: {
                type: "object",
                properties: {
                    date_from: {
        ...
        },
        {
            ...
            description: "Get full detail for a single pre-meeting research brief by ID.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Create an Akiflow task from a meeting recording's action item. Bridges meeting action items into your task list.",
            inputSchema: {
                type: "object",
                properties: {
                    recording_id: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        switch (name) {
            case "get-tasks": {
                const { done = false, status, date_from, date_to, listId, priority, limit } = GetTasksSchema.parse(args);
                const response = await client.getTasks();
                let tasks = response.data.filter((t) => t.deleted_at === null && t.trashed_at === null);
                // Apply filters
                if (done !== undefined) {
                    tasks = tasks.filter((t) => t.done === done);
                }
                if (status) {
                    tasks = tasks.filter((t) => t.status === parseInt(status));
                }
                if (date_from) {
            ...
            case "add-task": {
                ...
                if (params.duration)
                    task.duration = params.duration * 60; // Convert to seconds
                if (params.priority)
                    task.priority = parseInt(params.priority);
                if (params.status)
                    task.status = parseInt(params.status);
                if (params.listId)
                    task.listId = params.listId;
                if (params.tags_ids)
                    task.tags_ids = params.tags_ids;
            ...
            case "get-events": {
                const { limit, calendar_id, date_from, date_to } = GetEventsSchema.parse(args);
                const response = await client.getEvents();
                let events = response.data.filter((e) => e.deleted_at === null);
                // Filter by calendar
                if (calendar_id) {
                    events = events.filter((e) => e.calendar_id === calendar_id);
                }
                if (date_from) {
            ...
            case "get-time-slots": {
                ...
                let slots = response.data.filter((s) => s.deleted_at === null);
                if (date_from) {
                    slots = slots.filter((s) => s.start_time.substring(0, 10) >= date_from);
                }
                if (date_to) {
                    slots = slots.filter((s) => s.start_time.substring(0, 10) <= date_to);
                }
                // Sort by start time
                slots.sort((a, b) => {
            ...
            case "get-recordings": {
                ...
                recordings = recordings.filter((r) => !r.trashedAt);
                if (date_from) {
            ...
            case "create-task-from-action-item": {
                const { recording_id, action_item_id } = CreateTaskFromActionItemSchema.parse(args);
                const result = await client.createTaskFromActionItem(recording_id, action_item_id);
                return {
                    content: [
                        {
                            type: "text",
